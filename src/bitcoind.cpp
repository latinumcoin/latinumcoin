...
bool AppInit(int argc, char* argv[])
{
    ...
    try
    {
        ...
        // Parameters
        //
        // If Qt is used, parameters/latinumcoin.conf are parsed in qt/bitcoin.cpp's main()
        ...
        {
            // First part of help message is specific to latinumcoind / RPC client
            std::string strUsage = _("Latinumcoin Core Daemon") + " " + _("version") + " " + FormatFullVersion() + "\n\n" +
                _("Usage:") + "\n" +
        ...
        for (int i = 1; i < argc; i++)
            if (!IsSwitchChar(argv[i][0]) && !boost::algorithm::istarts_with(argv[i], "latinumcoin:"))
                fCommandLine = true;
        ...
        if (fDaemon)
        {
            fprintf(stdout, "Latinumcoin server starting\n");
